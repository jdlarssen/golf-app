...
-- ── 4–8. The 5 RLS helpers all have an explicit search_path in proconfig ──────
...
select ok(
  exists(
    select 1 from pg_proc p
    join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'same_flight'
      and p.proconfig::text like '%search_path%'
  ),
  '#671: same_flight() has SET search_path in proconfig'
);
