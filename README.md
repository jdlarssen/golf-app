...
- Your own cup. Run a team-vs-team Ryder Cup among friends without needing a club: name the two teams, generate the matches from your friends, then start and finish it yourself. When you generate the matches, every session comes with a match count you can dial down — run twelve singles off sixteen-player teams and the rest sit out that session. A personal cup holds up to thirty-six matches and forty players, which fits a full Ryder Cup line-up with captains on top; need more and that's what a club cup is for. You can also hand the picking to the players themselves: put each participant on a team, name one captain per side, and open a session at a time. Each captain fills their own ordered line-up without seeing the other side's, and the moment both have submitted, the matches appear and everyone is told. The organiser still sees both drafts, can submit on a captain's behalf, and can unlock a submitted line-up before the reveal. Skip the captains and the cup behaves exactly as it did before.
