...
describe('catalogForPlayerCount', () => {
  useFreshModules();
  ...
  it('bruker den delte fitsPlayerCount-regelen', () => {
    const all = [
      entry('stableford'),
      entry('singles_matchplay'),
      entry('best_ball'),
      entry('wolf'),
      entry('greensome_matchplay'),
      entry('skins'),
    ];
    const slugsFor = (n: number) =>
      catalog()
        .catalogForPlayerCount(all, n)
        .map((e) => e.slug);

    // 2 spillere: singles matchplay og best ball passer, wolf (3-5) gjør ikke.
    expect(slugsFor(2)).toEqual([
      'stableford',
      'singles_matchplay',
      'best_ball',
      'skins',
    ]);
    // 4 spillere: wolf og greensome åpner, singles matchplay (eksakt 2) lukker.
    expect(slugsFor(4)).toEqual([
      'stableford',
      'best_ball',
      'wolf',
      'greensome_matchplay',
    ...
    // 3 spillere: best ball krever partall.
    expect(slugsFor(3)).toEqual(['stableford', 'wolf', 'skins']);
  });

  it('gir tom liste for et tomt spill', () => {
    expect(catalog().catalogForPlayerCount([entry('stableford')], 0)).toEqual([]);
  });
});
