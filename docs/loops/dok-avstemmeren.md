...
| C3 | CLAUDE.md → Samarbeidsmodell: SQL-tilgang beskriver MCP + staging-først + prod-luke | grep «Supabase MCP» CLAUDE.md; `.claude/hooks/mcp-guard.sh` finnes | 2026-09-14 (2 treff «Supabase MCP»; mcp-guard.sh finnes) |
...
| C7 | Levende styringsdokumenter (CLAUDE.md, AGENTS.md, docs/user-flows.md, docs/test-discipline.md, docs/agent-discipline/, docs/loops/ — men IKKE `docs/loops/logg/`, som er #1110-tavlas månedsarkiv) siterer kun fil-stier som finnes | `node scripts/check-doc-paths.mjs` (skanneren eier dokumentlista og filtrerings-reglene, #1554; historiske dokumenter — `docs/plans/`, `docs/audits/`, `docs/superpowers/specs/`, CHANGELOG — er punkt-i-tid-arkiv og skannes IKKE; `.forge/` er ute av repoet siden #1931) | 2026-09-14 (`sjekket 195, brutt 0`) |
| C8 | CLAUDE.md → Branch + PR-flyt: hook-inventaret beskriver hva vaktene faktisk gjør | grep `emit_deny`/`emit_ask` i `.claude/hooks/bash-guard.sh`; grep main-vakt i `.githooks/pre-push`; grep `Refs` i `.githooks/commit-msg` | 2026-09-14 (pre-push blokkerer main ✓, commit-msg krever Refs #N ✓, `--no-verify` + `--squash` = DENY ✓; **avvik:** `git push --force` er `emit_ask`, ikke DENY — regel-avvik → #2027, aldri auto-harmonisert) |
| C9 | CLAUDE.md → intro + hosting: «over 20 spillemodi» og Vercel-region `arn1` | tell medlemmer i `GameMode`-unionen (`lib/scoring/modes/types.ts`); `grep regions vercel.json` | 2026-09-14 (22 modi → «over 20» ✓; `vercel.json:3` = `["arn1"]` ✓) |
