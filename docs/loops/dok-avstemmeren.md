...
| C8 | docs/pr-workflow.md → Branch + PR-flyt: hook-inventaret beskriver hva vaktene faktisk gjør | grep `emit_deny`/`emit_ask` i `.claude/hooks/bash-guard.sh`; grep main-vakt i `.githooks/pre-push`; grep `Refs` i `.githooks/commit-msg` | 2026-09-14 (pre-push blokkerer main ✓, commit-msg krever Refs #N ✓, `--no-verify` + `--squash` = DENY ✓; **avvik:** `git push --force` er `emit_ask`, ikke DENY — regel-avvik → #2027, aldri auto-harmonisert) |
