...
`/forge:contract`s «load prior decisions»-steg leter i issue-kommentarene, ikke på disk: iterér de aktuelle issuene — det aktuelle issuet selv, issuene det lenker til (`Refs`/`Part of`/omtaler i body), og åpne issues med samme `area:`-label — og hent kommentarene per issue med `gh api repos/jdlarssen/golf-app/issues/N/comments`, og se etter headeren «📋 Forge-kontrakt tilgjengelig». Bruk ALDRI `gh search … in:comments` — den returnerer tomt for kommentar-innhold (verifisert 2026-07-07, jf. dok-avstemmeren C4). Begrensning, sagt høyt: uten `ls .forge/contracts/` finnes ingen enkel tvers-av-alt-oversikt over lukkede kontrakter — en tavle over kontrakt-kommentarer er en utsatt idé (#1931). Gjett ikke; si i kontrakten hvilke issues du faktisk leste.
