...
- [ ] **K1 — Typer + uttømmende maps:** `npm run build` exit 0. `patsome` lagt til i `GameMode`/`MODE_LABELS`/`GameModeConfig`/`ModeResult`, `computeLeaderboard`-switch, `modeValidators`+`bruttoHelperFor`+`MODE_GUIDE`, `TeamSizeSelector`/`ReadyStep`-maps, og evt. `GameRow`-mirror-unions. *Evidens: build-output + grep.*
- [ ] **K2 — Scoring-modul:** `lib/scoring/modes/patsome.ts` `compute()` — segment-bytte (1–6/7–12/13–18), 4BBB MAX-regel, greensome 60/40, foursomes 50 %, netto/brutto, segment-delsummer, total, ranking m/ tie-break. *Evidens: `patsome.test.ts` grønn.*
- [ ] **K3 — Type A unit-tester:** `patsome.test.ts` dekker: shape; 4BBB MAX (begge varianter contributor); greensome 60/40-allowance (kjent CH/SI); foursomes 50 %-allowance; segment-overgang ved hull 6→7 og 12→13; netto-vs-brutto-flip; segment-delsummer + total; pending/uspilt hull (0 poeng); fler-lags-ranking + `tiedWith`; tom/draft-state (n≠2 krasjer ikke); kaptein = lex-min. *Evidens: test-tall.*
- [ ] **K4 — Validator + regresjon:** `validatePatsome` (<4 → `min_players_for_mode`; lag ikke à 2 → `team_balance`; `duplicate_player`) + `parseGameMode` + `modeValidators`. Regresjonscases i `gamePayload.test.ts`. *Evidens: test-tall.*
- [ ] **K5 — Migrasjon + tee-starter-tabell:** `0055_patsome.sql` seeder format-row «Patsome» + klubb-mapping (sekundær) + oppretter `patsome_tee_starters` m/ RLS. `lib/database.types.ts` regenerert. *Evidens: `grep patsome lib/database.types.ts` + migrasjons-fil.*
- [ ] **K6 — Hybrid scorekort-inntasting:** hull 1–6 viser per-spiller-kort, hull 7–18 viser lag-ball-kort (kaptein-eid). Segment-banner per hull. *Evidens: kode (collapse-betingelse) + Playwright/preview-røyktest.*
- [ ] **K7 — Tee-starter (foursomes-segment):** per-lag velger på hull 13 når ikke satt; «X slår ut»-hint på hull 13–18 (oddetall = valgt, partall = makker); greensome har kun banner. `setPatsomeTeeStarter` validerer lag-medlemskap. *Evidens: action-test + preview.*
- [ ] **K8 — Leaderboard + podium:** `PatsomeView` (rangering + segment-delsummer + per-hull-rutenett + reveal-aware) + `PatsomePodium` wiret via `renderPatsome`. Type C render-test. *Evidens: `PatsomeView.test.tsx` grønn + preview.*
- [ ] **K9 — CHANGELOG + versjon:** 1.50.0 → 1.51.0 (MINOR) + CHANGELOG-oppføring (1.51.y-serie, forrige serie wrappet i `<details>` ved behov). commit-msg-hook passerer. *Evidens: hook-pass + diff.*
