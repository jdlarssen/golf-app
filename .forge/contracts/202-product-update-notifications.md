...
### 1. Datamodell (migrasjon `0035_product_updates.sql`)
...
- [ ] **K1:** Migrasjon `0035_product_updates.sql` finnes og kjøres rent. Tre nye DB-objekter: `product_updates`-tabell, `product_update_digests`-tabell, `users.product_updates_unsubscribed_at`-kolonne. Notifications-kind-CHECK utvidet med `product_update`. RLS-policy på `product_updates` lar alle innloggede SELECT.
...
- [ ] **K11:** Version bumpet `1.20.0` → `1.21.0` (minor — ny bruker-synlig funksjon). CHANGELOG-oppføring med stakeholder-tagline. 1.20.y-serien wrappet i `<details>`.
...
| Fil | Status | Hva |
|---|---|---|
| `supabase/migrations/0035_product_updates.sql` | NY | Tabeller + kolonne + CHECK-utvidelse + RLS |
...
| `package.json` | ENDRET | Bump `1.20.0` → `1.21.0` |
| `CHANGELOG.md` | ENDRET | Ny `## 1.21.y`-tema-heading + entry; wrap `1.20.y` i `<details>` |
