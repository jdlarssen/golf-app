...
# DRIFT-KORRIGERING (2026-09-01, mot HEAD 5b955a7b)

Kontrakten over ble skrevet før N6a (#1854) og N6b (#1855/#1873) landet. Full
drift-tabell: `.forge/contracts/n6c-drift-mot-head.md`. **Ved konflikt vinner
dette avsnittet.** Tre spec-påstander var GONE, ni DRIFTED.

## Rettelser som endrer designet

**1. Den optimistiske låsen finnes ikke (GONE).** `endGameCore.ts:221-224` er
`.update({status,ended_at}).eq('id', gameId)` — ingen `.eq('status','active')`,
ingen `.select()`. Et 0-rads-skriv gir `error===null` og hele halen kjører mot et
spill som fortsatt er `active`. Låsen **bygges** i dette arbeidet, i SAMME commit
som hale-uttrekket (uten den mister vi `status!=='active'`-gatens utilsiktede
dobbeltkjørings-vern). Semantikk: 0 rader = allerede avsluttet = **idempotent
suksess** (`alreadyFinished: true`), speilet fra `startScheduledGameCore`s
`started`-boolean — ikke en feil. Et legitimt dobbelttrykk skal ikke gi feilmelding.

**2. Cup-ness kommer ikke fra `tournament_id` i webben (GONE).** endGameCore har
null treff på `tournament_id`. Cup-ness kommer via to caller-styrte kanaler:
hvilken klient caller sender (`lib/cup/actions.ts:348` = service-role) og
`suppressPerGameNotifications` (:354, hardkodet `true`). Begge MÅ forbli
parametere gjennom uttrekket. `finishDerivedGames` gater på `games.source_game_id`
og kjører ubetinget. — **Appens** cup-gate bruker derimot `games.tournament_id`
(finnes i `gameBundle.ts:59` som `tournamentId`, hentet men ubrukt i dag). Det er
en annen sjekk og er fortsatt riktig.

**3. Markøren settes FØRST, ikke sist (avvik fra kontrakten).** Kontrakten sa
«setter `finish_pipeline_at` som SISTE steg». Det gir at-least-once. Men
`notifyAchievementUnlocks` er **ikke** idempotent (`notify()` er en bar INSERT og
prod har ingen unik indeks på `public.notifications` — kun `notifications_pkey`
+ tre ikke-unike btrees), `generateAndPersistRoundReport` fakturerer et nytt
Anthropic-kall per kjøring, og finish-mailen er re-sendbar. Dobbel-varsling er
verre enn et manglende runde-referat. Derfor: **vinn-raden-først**, nøyaktig
husmønsteret fra `lib/notifications/autoStartBlocked.ts:67-82`:
`.update({finish_pipeline_at: now}).is('finish_pipeline_at', null).select('id').maybeSingle()`
→ 0 rader = noen andre eier kjøringen = returner. At-most-once for de
ikke-idempotente stegene.

**4. Sweepen er pg_cron + pg_net, ikke Vercel-cron (DRIFTED).** #502 er bevisst
UTE av `vercel.json` (Hobby gir 1/døgn). Ny jobb = ny `cron.schedule(...)`-migrasjon
= prod-DB-endring. Kadens arves: `* * * * *` med `where exists`-gate, verifisert
live i prod (`cron.job` har én rad, jobid 1, active). **Ett minutt << 15 min ⇒
ingen `after()`-fallback.** Ruta må være **POST** (pg_net kan ikke GET).
`cron_secret` finnes alt i Vault ⇒ ingen nye eier-steg utover selve luka.
*Valgt:* egen jobb + egen rute (ikke utvide `start-scheduled-games`) — skilte
bekymringer, egen gate, egen `[finishPipeline]`-logg; eier-luka trengs uansett
for markørkolonnen, så andre `cron.schedule` koster null ekstra eier-steg.

**5. Mail-hjelperen heter `sendGameFinishedNotification` og KASTER (DRIFTED).**
Best-effort bor i call-site (`endGameCore.ts:307-324`, `Promise.allSettled`).
Flyttes kallet, MÅ allSettled følge med — ellers begynner avslutning å kaste.

**6. Ingen `endGameCore.test.ts` finnes (DRIFTED).** Dekningen er indirekte via
`app/[locale]/admin/games/[id]/actions.test.ts:534` (8 tester). De fire
post-steg-hjelperne er **ikke** mocket — de kjører ekte, nøytralisert av
tilfeldigheter (server-only-alias, tom admin-kø, manglende `ANTHROPIC_API_KEY`).
`buildSupabaseMock` er FIFO ⇒ omrokering av DB-kall tildeler canned-svar til feil
query, stille. `expect(notifyMock).toHaveBeenCalledTimes(2)` (:830) henger på at
`notifyAchievementUnlocks` finner null momenter. **Skriv kjernens egne tester FØR
koden flyttes.** Baseline: 26/26 i fila, 38/38 med syncDerivedGamesStatus +
cron-ruta.

**7. `finish_pipeline_at` blir skrivbar av oppretteren i det den finnes.**
`games creator update` gir blankofullmakt på alle kolonner. Skal kun serveren
sette den, kreves en guard-trigger (speil 0117). *Valgt:* legg til guard-triggeren
— markøren styrer varsling og penger (Anthropic-kall), så en klient som kan nulle
den kan tvinge re-varsling.

**8. Native tema + primitiver (DRIFTED).** Flater som henger på GameHome bruker
statisk `ui`/`COLORS` som `OrganiserSection.tsx:13-16`, **ikke** `useTheme()`
(#1866 er utsatt eier-valg). Checkbox og picker **finnes ikke** — bygges fra
`SelectRow`/`Chips`/`ToggleRow` i `components/create/primitives.tsx`.
Simulator-kvirk: `Switch` tar ikke tapp, den må dras.

## Empirisk avklart på staging (2026-09-01) — risiko #4 lukket

Probet mot torny-staging med ekte JWT for en **ikke-admin oppretter** på et
aktivt spill (`681ca24b`, side på, ld=1 ctp=1). Fikstur restaurert etterpå.

| Probe | Resultat |
|---|---|
| A: `game_side_winners`-upsert m/ `return=representation` mens `status='active'` | **201 + rader** — husets trap-2-idiom (`.select()`) er trygt her |
| B: creator SELECT av winners mens `active` | **200 + rader** (creator-all OR-er inn) |
| C: flipp `?id=eq.X&status=eq.active` m/ representation | **200 + rad** — låsen virker fra RLS-klient |
| D: re-flipp når alt finished | **200 + `[]`** — rent 0-rads-signal for idempotens |
| E: re-upsert winners på finished spill | **200 + rad** — retry-stien virker |

⇒ Native kan trygt bruke `.select()` på begge skriv og skille «nektet» fra
«allerede utført» uten oppfølgings-SELECT.

## Produktvalg som skal fram i PR-en

Halen kjører via sweep ⇒ resultat-sammendrag, differensialer, achievements,
runde-referat og «Resultatet er klart»-mail kommer **inntil ett minutt etter** at
arrangøren avslutter fra appen (web er uendret synkron). Alternativet — appen
kaller en server-rute som kjører halen der og da — gir umiddelbart resultat, men
mister sweepens sikkerhetsnett hvis nettet ryker rett etter flippen. Presenteres
som `## Alternativer (produktvalg)` med A/B/C (C = begge). **PR-en auto-merges
aldri** (prod-DB-migrasjon + produktvalg).

## Uendret fra kontrakten

Skriverekkefølge (winners FØR flipp), peer-gaten aldri relaksert av
`allowMissing`, «Ingen kvalifiserte» ⇒ `winner_user_id: null`, `position` =
hull-slot, dedikert bekreftelses-flate, cup henvises til web, jest-paritet,
`[no-changelog]` på native-commits, prod-brannmuren #1074.
