...
## Success Criteria
- [x] **C1 — Schema applyt:** `friendships` (+RLS, secdef-only writes), `users.friend_code` (notnull/unique/format), `games.let_friends_skip_gate` (boolean default false), 5 RPCer (`send_friend_request`, `send_friend_request_by_email`, `respond_friend_request`, `remove_friend`, `connect_via_friend_code`) security definer m/ anon revoked, `friend_request`+`friend_accepted` kinds. *Verifiser:* MCP `execute_sql` mot `information_schema`/`pg_proc`/`pg_policies`/`pg_constraint` + `lib/database.types.ts` har feltene; `npx tsc --noEmit` + `npm run build` grønt.
  → **Bevis:** migrasjon 0077 applyt via MCP. `execute_sql`: friendships_tbl=1, rls=true, 1 policy; friend_code notnull + unik + 0 manglende; let_friends_skip_gate=1; secdef_rpcs=5; friend_kinds=1; anon_can_execute=0. `database.types.ts` regenerert (`grep` traff friendships/friend_code/let_friends_skip_gate/connect_via_friend_code). tsc=0, build grønn. Commit `626e396`.
- [x] **C2 — Tre veier å legge til venn:** (a) co-player-forslag «Legg til» → pending; (b) e-post: eksisterende bruker → forespørsel, ukjent e-post → invitasjons-tilbud gjenbruker e-posten (`sendFriendInvite`); (c) `/venner/legg-til/[friend_code]` → accepted-relasjon direkte. *Verifiser:* Playwright + `execute_sql` (rad-effekter) + RPC-retur.
  → **Bevis:** [`app/profile/venner/actions.ts`](app/profile/venner/actions.ts): `sendFriendRequest` (forslag), `addFriendByEmail` (`not_found` → redirect `?invite_email=` → `sendFriendInvite`-tilbud i [`page.tsx`](app/profile/venner/page.tsx)). Lenke: [`app/venner/legg-til/[code]/`](app/venner/legg-til/[code]/page.tsx) → `connect_via_friend_code` (accepted direkte). Commit `c08e708`. Live-flyt verifiseres av evaluator.
- [x] **C3 — Gjensidig forespørsel→godta + varsler:** forespørsel lager pending + `friend_request`-varsel til mottaker; mottaker godtar (Innboks/venner-side) → `accepted` + `friend_accepted`-varsel til avsender; avslå sletter raden. *Verifiser:* `execute_sql` (status-overgang) + notify-test/observasjon.
  → **Bevis:** `respondFriendRequest` (accept→`accepted`+notify requester; decline→DELETE). `notifyForStatus` sender `friend_request`/`friend_accepted` (best-effort). RPC `respond_friend_request` authz: kun addressee, kun pending. Notification-kinds + zod + kort-render + deeplink `/profile/venner` (chunk 3). Commit `c08e708`.
- [x] **C4 — Venneliste + fjern:** `/profile/venner` viser venner/innkommende/utgående/forslag; «Fjern» (inline bekreft) sletter accepted-relasjonen begge veier. *Verifiser:* Playwright + `execute_sql`.
  → **Bevis:** [`page.tsx`](app/profile/venner/page.tsx) seksjoner: innkommende (Godta/Avslå), Vennene dine (`ConfirmSubmit` to-tap fjern), Venter på svar (trekk), Forslag (Legg til), e-post, del-lenke. `getFriendData` partisjonerer via testet `friendGraph`. `removeFriend` → RPC `remove_friend` (begge retninger). Commit `c08e708`.
- [x] **C5 — Autocomplete-union (#408):** lag-påmeldings-autocomplete viser nå `venner ∪ co-players`; en venn uten delt spill dukker opp. *Verifiser:* `getTeamCandidates.test.ts` (ny case) + Playwright på `/signup/[shortId]/team`.
  → **Bevis:** [`getTeamCandidates.ts`](lib/users/getTeamCandidates.ts) = `getFriendIds ∪ getCoPlayerIds`, dedup. 4 nye unit-tester (union/dedup, friend-uten-delt-spill, tom uten query, e-post-filter+sort) — alle grønne. `TeamRegistrationForm` uendret. Commit `a348ade`.
- [x] **C6 — «Slipp venner direkte inn»:** veiviseren har checkbox kun under `manual_approval`, lagres som `let_friends_skip_gate`; en **venn** av arrangøren melder seg **direkte** på et slikt spill, en **ikke-venn** må be om å bli med. *Verifiser:* `execute_sql` (kolonne satt) + `getDiscoverableGames`/signup-test + Playwright begge stier.
  → **Bevis:** [`RegistrationSection.tsx`](app/admin/games/new/sections/RegistrationSection.tsx) checkbox kun under `manual_approval`; [`gamePayload.ts`](lib/games/gamePayload.ts) tvinger `false` utenfor `manual_approval`; persistert i create+edit-actions. [`registerForOpenGame`](app/signup/[shortId]/actions.ts): `canDirectJoin` += venn ∧ skip-gate (server-verifisert via `getFriendIds`); ikke-venn → uendret request. Signup-side viser «Meld meg på» for venn. Commit `c494a8a`.
- [x] **C7 — «Fra vennene dine»-discovery:** venners `open`/`manual_approval`-spill vises i egen seksjon på `/finn-turneringer` + hjem; `invite_only` vises **aldri**; CTA korrekt (direkte ved `open` el. skip-gate+venn, ellers be-om); dedup vs klubb/open. *Verifiser:* `getDiscoverableGames.test.ts` (nye cases) + Playwright.
  → **Bevis:** [`getDiscoverableGames.ts`](lib/games/getDiscoverableGames.ts) `friendGames`-seksjon (created_by ∈ venner, mode ∈ open/manual_approval, dedup vs klubb + ekskluderes fra open). `joinMode` = direct (open ∨ skip-gate) / request. 5 nye test-cases (request, skip-gate→direct, invite_only ekskludert, dedup) — grønne. [`HomeDiscoverySection.tsx`](app/HomeDiscoverySection.tsx) «Fra vennene dine» + CTA. Commit `c494a8a`.
- [x] **C8 — Veiviser kompis-venn-quick-add:** under kompis-arrangement kan oppretter legge til venner (`venner ∪ co-players`) som spillere i roster-steget; klubb-grenen uendret. *Verifiser:* Playwright + komponent-ref.
  → **Bevis:** `FriendQuickAdd` i [`GameWizard.tsx`](app/admin/games/new/GameWizard.tsx) steg 4, kun `intent === 'kompis'`, leser `friendPlayerIds`. Admin-flyt: alle venner er i `players`. /opprett-spill: [`getFriendPlayerOptions.ts`](lib/friends/getFriendPlayerOptions.ts) unionerer venner inn i `players` (admin-client forbi users-RLS) så venner uten delt spill også når quick-add. Commit `c494a8a`.
- [x] **C9 — Ingen regresjon + gates grønne + docs/flyt:** eksisterende discovery/signup/#442-oppførsel uendret; `npm run build` grønn, berørte co-lokerte tester grønne, `02-bli-med-i-spill-fremtid` regenerert, `user-flows.md` oppdatert, ny norsk copy humanisert. *Verifiser:* `npm run build` + `npx vitest run` + `git diff` + flow-regen.
  → **Bevis:** full `npx vitest run` = **221 filer / 2687 tester passed**. tsc=0, `npm run build` grønn (35 sider). Ingen eksisterende RLS/policy endret (kun additiv 0077). Flyt-SVG oppdatert + PNG regenerert (qlmanage); `user-flows.md` §0 + P5; README «What you get» friends-bullet; flows/README #369 ✓. Ny copy humanisert (em-dash «Sendt — venter» → «Venter på svar»). Commit `d7a182c`.
