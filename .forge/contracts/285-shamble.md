...
- [x] **K1 — Typer + uttømmende maps:** ✅ `shamble` i `GameMode`/`MODE_LABELS`/`GameModeConfig`/`ModeResult`, `computeLeaderboard`-switch, `modeValidators`-Record, `MODE_GUIDE`-Record, `bruttoHelperFor`-switch, `TeamSizeSelector.ENABLED_COMBOS`, `ReadyStep.MODE_SUMMARY_LABELS`, og `GameRow`-mirror-union i `app/games/[id]/page.tsx`. **Evidence:** `npm run build` → «Compiled successfully» + 30/30 static pages; tre completeness-feil fanget og fikset (`MODE_SUMMARY_LABELS`, `ENABLED_COMBOS`, `GameRow`-union). Commit `79068b4`.
- [x] **K2 — Scoring-modul:** ✅ `lib/scoring/modes/shamble.ts` `compute()` — best-N-sum per hull (N=1/2/3), netto/brutto via `effectiveFor`, pending uten carryover, lag-ranking via `rankTeams`-cascade. **Evidence:** [`shamble.ts`](lib/scoring/modes/shamble.ts) committet `312409f`; K3 grønn.
- [x] **K3 — Type A unit-tester:** ✅ `shamble.test.ts` 19 cases: shape-ekko, best-1/2/3 av 4, best-2/1 av 3, likhet på grensa (begge telles + alle-like → nøyaktig N), netto-vs-brutto-flip, pending (teamScore null, holesCounted 0), count-klamp (Champagne 3 på lag-3 → alle telles), Shamble-preset låser count=2, fler-hull-total, to-lags rang + `tiedWith`, defensive defaults, perPlayer-dekning. **Evidence:** `npx vitest run lib/scoring/modes/shamble.test.ts` → 19 passed.
- [x] **K4 — Validator + regresjon:** ✅ `validateShamble` (team_size 3/4, teams_count-balanse, Shamble låser count=2, Champagne default count 2) + `parseGameMode` + `modeValidators`. **Evidence:** 7 regresjonscases grønne i `gamePayload.test.ts` (preset-lås, Champagne-3/brutto, default-count, team_balance, manglende team_size, duplikat, draft-partial). Commit `79068b4`.
- [ ] **K5 — Migrasjon:** `0055_shamble.sql` seeder ÉN format-rad «Shamble / Champagne Scramble» + klubb-mapping (sekundær, ikke primary). **Status:** fil committet (`79068b4`), matcher 0054-idiom. **DB-apply utsatt** til wizard+leaderboard er klare (single-prosjekt-DB; unngå halv-bygd format i prod — samme avveining som Nines K6). Kjøres ved verifisering.
- [x] **K6 — Wizard:** ✅ Én datadrevet flis (auto fra format-rad); `ShambleSetup` med lagstørrelse-radio (3/4), variant-radio (Shamble/Champagne), count-velger (1/2/3) kun ved Champagne, netto/brutto (default netto). `isShamble` wiret gjennom `useGameFormState` som `isTexas`; `TeamSize` utvidet til 1|2|3|4; `TeamsAssignmentSection` rendrer `slotCount = teamSize` slots → lag-tildeling for 3 OG 4. **Evidence:** `ShambleSetup.test.tsx` 1 render-test grønn (variant→count-visibility); `npm run build` 30/30. Commit `1ccfb5c`. **Live Playwright utsatt** (single-prosjekt-DB: migrasjon ikke kjørt mot delt DB før merge — samme som Nines K6); dekket av render-test + build.
- [x] **K7 — Leaderboard + Type C render-test:** ✅ `ShambleView` (lag-rangering + per-hull-rutenett + counted-✓-highlight + pending-merking + reveal-aware) + `ShamblePodium` (topp-3-lag + rest-liste, konfetti) wiret via `renderShamble` (ekte team_number, `kind`-guard → notFound). **Evidence:** `ShambleView.test.tsx` 1 render-test grønn (lag-rader + per-hull-rutenett + reveal-skjul); `npm run build` 30/30. Commit `c5c9937`. **Live Playwright utsatt** (samme DB-constraint).
- [x] **K8 — CHANGELOG + versjon:** ✅ 1.50.0 → 1.51.0 (MINOR) + CHANGELOG 1.51.y-serie åpnet, 1.50.y wrappet i `<details>`. **Evidence:** release-commit `82f4850` staget package.json + package-lock.json + CHANGELOG.md; commit-msg-hook passerte (`feat(`-prefiks krever bump+CHANGELOG).
