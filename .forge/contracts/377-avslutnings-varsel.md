...
- [x] **K1** — Enkeltspill-avslutning bekreftet: in-app `game_finished` for ALLE deltakere (også aktive), mail kun off-app, ingen blanket-mail. _Evidens: `endGame` ([actions.ts:340](app/admin/games/[id]/actions.ts)) og `endGameWithSideWinners` ([avslutt/actions.ts:195](app/admin/games/[id]/avslutt/actions.ts)) kaller `notifyPlayersGameFinished(players!, ...)`; `notify()` ([notify.ts:41](lib/notifications/notify.ts)) inserter alltid in-app og `shouldAlsoSendMail` gater kun mail; mail filtreres `recipients.filter(r => sendMailByUserId.get(r.userId) === true)` ([actions.ts:359](app/admin/games/[id]/actions.ts)). Allerede Type A-testet i [events.test.ts](lib/notifications/events.test.ts)._
- [x] **K2** — Ny `cup_finished`-kind wiret gjennom alle uttømmende steder; `npm run build` grønn (tsc håndhever at ingen switch/Record glemmes). _Evidens: union + `cupFinishedSchema` + schemas-map i [types.ts](lib/notifications/types.ts); `EMOJI` (`🏁`) + `buildCardContent`-case i [NotificationCard.tsx](components/notifications/NotificationCard.tsx); `buildDeeplink`-case i [InboxClient.tsx](app/innboks/InboxClient.tsx); CHECK i [0069](supabase/migrations/0069_cup_finished.sql). `npm run build` → «✓ Compiled successfully in 2.7s»._
- [x] **K3** — `notifyParticipantsCupFinished` fyrer in-app `cup_finished` for alle cup-deltakere; returnerer per-deltaker `shouldAlsoSendMail`-map; fail-closed ved notify-rejection. _Evidens: funksjon i [events.ts](lib/notifications/events.ts) + 4 Type A-tester i [events.test.ts](lib/notifications/events.test.ts) (map, fail-closed, log-prefix, tom liste) — 8/8 grønne._
- [x] **K4** — `finishTournament` sender in-app til alle og mail KUN til off-app-deltakere; ingen blanket-mail til alle. _Evidens: [cup/actions.ts](lib/cup/actions.ts) — `notifyParticipantsCupFinished(recipients, { id, name }, 'finishTournament')` → `recipients.filter(r => sendMailByUserId.get(r.user_id) === true)` før mail-fan-out (tidligere `recipients.map(...)` uten gating)._
- [x] **K5** — Migrasjon `0069_cup_finished.sql` skrevet + applisert (Supabase MCP; additiv CHECK-utvidelse, trygg). _Evidens: `apply_migration` → `{"success":true}`; fil i `supabase/migrations/0069_cup_finished.sql`._
- [x] **K6** — Innboks-kort for `cup_finished` viser fornuftig tittel/detalj + deeplinker til `/cup/{tournament_id}`; norsk copy kjørt gjennom humanizer. _Evidens: `buildCardContent`-case (tittel «Cupen er ferdigspilt», detalj = tournament_name); `buildDeeplink` → `/cup/{tournament_id}` (ruta = `PublicCupPage`, deltaker-facing); humanizer-kjøring endret «avgjort»→«ferdigspilt» (tie-safe)._
- [x] **K7** — Version-bump (MINOR) + CHANGELOG-oppføring i samme commit som feature. _Evidens: `package.json` 1.72.0 + CHANGELOG 1.72.y-serie (1.71.y wrappet i `<details>`); commit `f7067df`._
- [x] **K8** — Gates grønne: `npm run build` + `npx vitest run` (berørte filer). _Evidens: build «✓ Compiled successfully»; `npx vitest run lib/notifications lib/cup lib/mail` → 24 filer / 202 tester grønne + events.test.ts 8/8._
