...
- [x] **K6 — Opt-in UI:** «Registrer putter»-bryter vises kun i fangst-formater, default av, persister i `localStorage` per game; putts-felt avsløres når på og skriver via `writeScore`. *Evidens:* staging-klikkrunde (stableford-spill `fab70b1a`, innlogget E2E-spiller): bryter rendret `aria-checked=false`, 0 putts-felt; etter klikk → `aria-checked=true`, `localStorage['torny:putts:…']='1'`, 2 PuttsField (én per spiller). Tastet putter (Test Admin=2, Test Spiller=1) → DB `scores.putts` = 2/1 OG `strokes` 4/5 **bevart** (merge live-bekreftet — K3 også). Skjermbilde tatt. Testrad ryddet.
- [x] **K7 — Putte-snitt:** Statistikk-fanen viser putte-snitt fra kvalifiserende runder; tom-tilstand når ingen putts. *Evidens:* `puttsStats.test.ts` (6, math + kvalifiserings-logikk) + `PuttsStatPanel.test.tsx` (2: populert snitt/beste/runder + tom-tilstand) grønne; side-wiring `tsc` + `npm run build` grønn. ⚠️ Live populert skjermbilde ikke mulig: ingen staging-bruker har en ferdig 18-hulls-runde (verifisert via SQL) — å seede en komplett ferdig runde på delt staging er for invasivt; render-test + datalag er oracle-et her.
- [x] **K8 — Ingen RLS-regresjon:** putts arver `scores` rad-RLS (innsendt scorekort låser putts også); ingen ny kolonne-policy nødvendig. *Evidens:* `putts` er en kolonne på `scores`, styrt av eksisterende rad-policies (0002 INSERT/UPDATE gater på «player not submitted», ikke per kolonne); RPC er `security invoker` → RLS gjelder; putts rir nøyaktig samme skrive-sti som `strokes` (live-verifisert i K6 at entered_by-gating virker). Ingen ny authz-flate.
...
- [x] **K10 — Prod-rollout (CONFIRM-GATED):** `0123` (kolonne + RPC) påført prod ETTER eksplisitt eier-bekreftelse («proceed with migration + merge»). *Evidens:* prod-SQL → `putts` integer nullable; CHECK 0..10; RPC `p_putts integer DEFAULT NULL`; `rpc_overload_count=1` (ingen tvetydig overload). Bakoverkompatibel → deployet prod-kode virker til ny kode deployes.
