...
- [x] **K2 — Bespoke subtitles parametrisert.** `soloStrokeplay`, `soloStableford`, `teamStableford`, `texasScramble` sine `subtitle` + `podiumSubtitle` bruker `{holes}` (begge språk). **+ best ball:** evaluatoren fanget at nøkkelen jeg først kalte foreldreløs `bestBall.subtitle` faktisk er `leaderboard.state4.subtitle` MED konsument (`State4View`, best_ball finished-reveal). Den er nå også `{holes}`-parametrisert + `State4View` tar `holesPlayed`-prop (commit d1e0c2d5). *Evidens: grep «subtitle/podiumSubtitle» + «18 hull» i begge kataloger = 0 treff.*
