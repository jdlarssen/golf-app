...
- [x] >4-spillers flight-løst solo-spill: Sekretariatet viser Flighter-seksjon med «Foreslå inndeling» (grupper på 4 i påmeldingsrekkefølge) + per-spiller-flytting; kun oppretter/admin. Verifikasjon: render-test (maks én) + actions-tester. — Evidens: FlighterSeksjon.tsx + flightActions.ts (suggestFlightAssignment/setPlayerFlight, authz admin/creator) + 8 actions-tester; admin page gates på eligibleForFlightAssignment + status scheduled/active.
