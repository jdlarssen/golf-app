...
- Skip-gaten: nå `lib/cup/actions.ts:272` (`if (pointsToWin !== null)`), ASSUMPTION-kommentaren fra `:262` — samme innhold som kontraktens :282–292.
- `cupStartedNotification.ts`: `pointsToWin: number` på `:36`, `formatNumber`-kallet på `:63` — som beskrevet.
- **K5 er UTDATERT (#1562):** ingen PATCH-bump/CHANGELOG-linje — `.changes/1444-<slug>.md`-notatfil (type fix) i stedet; commit-msg-hooken håndhever.
