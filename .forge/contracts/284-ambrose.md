...
- [~] Migrasjon 0055 mot prod — **utsatt til post-deploy** (bevisst). Wizard-format-grid er DB-drevet (`getFormatsForIntent` leser `format_intent_mapping`, `validateGameMode` sjekker `formats`); å seede raden FØR koden er deployet ville vist et ødelagt «Ambrose»-kort på live klubb-wizard. Korrekt rekkefølge: merge PR → main deployer → DERETTER `apply_migration 0055` via Supabase MCP + verifiser raden. Migrasjonsfil skrevet og verifisert mot 0054-malen; prod-baseline bekreftet (0054_nines er siste appliserte).
- [x] Admin kan opprette et Ambrose-spill (team_size 2 og 4) via wizarden; det vises med label «Ambrose». **Evidence:** ModeSelector «Ambrose»-tile + `ENABLED_COMBOS.ambrose = {2,4}`; `validateAmbrose` produserer mode_config; `npm run build` grønn. Visuell smoke gjenstår (krever deployet preview — se note nedenfor).
- [x] Leaderboard for et Ambrose-spill rendrer Texas-scramble-viewet med brutto + team-HCP + netto, og format-label «Ambrose». **Evidence:** `leaderboard/page.tsx` ruter `isScrambleFamily` → `renderTexasScramble({formatLabel: MODE_LABELS[game_mode]})`; `TexasScrambleView`/`Podium` tar `formatLabel`-prop (default «Texas scramble») → ambrose viser «Ambrose». Ingen ny Type C (gjenbruk per test-disiplin).
...
- [x] Versjons-bump 1.50.0 → 1.51.0 + CHANGELOG-oppføring i den bruker-synlige `feat`-commiten. **Evidence:** `package.json` 1.51.0; CHANGELOG «1.51.y — Ambrose» m. tagline + Teknisk; 1.50.y wrappet i `<details>`.
