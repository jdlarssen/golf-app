...
> **Avvik fra design (bevisst):** den foreslåtte belte-og-bukseseler-endringen — å bytte `startScheduledGame`s interne pending-defense (`users`-read) til RPC-en fra §1 — ble IKKE gjort. Begge nåværende kall-steder leser `users` med en klient som ser radene (auto-start: service-role; admin-knapp: admin-RLS), så byttet er ren no-op-hardening for et hypotetisk framtidig ikke-admin-kall. Droppet for å unngå unødvendig test-churn i `startScheduledGame.test.ts`. K4 (auto-start flipper uansett trigger) er uberørt — den oppfylles av service-role-klienten.
