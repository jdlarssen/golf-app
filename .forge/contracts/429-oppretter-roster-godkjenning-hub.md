...
- [ ] **K1 (migrasjon + RLS mot ekte auth):** `0072_invitations_creator_game_invite.sql` applisert. Creator game-scoped invitation INSERT + SELECT + DELETE-policyer finnes (`pg_policies`); eksisterende invitations-policyer urørt. Verifisert i rollback-txn med ekte ikke-admin `auth.uid()`: (T1) INSERT game-invite eget spill → tillatt; (T2) annet spill → 42501; (T3) SELECT egne game-invites → synlig; (T4) andres → 0 rader; (T5) DELETE egen → 1 rad, andres → 0. Ingen leftover-rader.
- [ ] **K2 (roster-actions):** `addExistingPlayerToGame` + `inviteEmailToGame` gater `requireAdminOrCreator`, forgrener `detailPath` på `isAdmin`, disposable-guard for ikke-admin e-post-invite. Oppretter legger til eksisterende + inviterer ny e-post på eget draft/scheduled-spill; lander på `/games/[id]/spillere`. Admin byte-identisk. Tester grønne (oppretter-gren + disposable-bounce + admin uendret + best-ball/game_locked).
- [ ] **K3 (withdraw):** `adminWithdrawPlayer` + `adminUndoWithdraw` gater `requireAdminOrCreator`, forgrener redirect. Oppretter trekker (og angrer) en spiller på eget aktivt `supportsWithdrawal`-spill. Admin byte-identisk. Tester grønne.
- [ ] **K4 (godkjennings-overstyring):** `adminApproveScorecard` gater `requireAdminOrCreator`, forgrener redirect (oppretter → `/games/[id]/spillere?status=approved`). Oppretter godkjenner et levert-men-ugodkjent scorekort på eget aktivt spill (ikke på andres). Idempotent + notify bevart. Admin byte-identisk. Tester grønne.
- [ ] **K5 (management-flate):** `/games/[id]/spillere` (AppShell, gated) registrert i build. draft/scheduled: roster + legg-til (getTeamCandidates) + inviter-e-post. active: roster m/ trekk + (ved require_peer_approval) «Leverte scorekort» m/ godkjenn-knapp. Roster lest via `getGameWithPlayers`. Ikke-eier ikke-admin → `/`.
- [ ] **K6 (game-home-inngang):** `/games/[id]` viser «Styr spillere» for oppretter ved scheduled/active (begge grener); skjult for ikke-oppretter. Eksisterende CreatorControls (Rediger/Slett/Avslutt) uendret.
- [ ] **K7 (hub):** `/klubbhuset` lister spill `created_by = meg` med StatusChip, hver rad → `/games/[id]`; tom-tilstand → opprett-lenke. Oppdagbar fra Hjem + Profil. Bunn-nav-fane bevisst utsatt til #392 (kommentar postet).
- [ ] **K8 (suite grønn):** `npm run lint` + `npm test` + `npm run build` + `tsc --noEmit` grønt. Eksisterende admin-invite/withdraw/approve-tester fortsatt grønne.
- [ ] **K9 (versjon + epic):** version-bump (MINOR-serie), CHANGELOG-oppføring (ny serie-seksjon, forrige wrappet i `<details>`), README-kapabilitets-linje. Humanizer-pass på ny norsk copy. Closing-kommentar #429 (Teknisk + Funksjonell). Epic #22 lukket. #392 kommentert. Reviewer-funn som ikke landerer → issues før merge.
