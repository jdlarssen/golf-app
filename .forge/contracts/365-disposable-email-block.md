...
- [ ] **K1 — Helper-logikk korrekt.** `isDisposableEmailDomain` returnerer `true` for kjente disposable-domener (case-insensitivt), `false` for normale (gmail/outlook/icloud/online.no/bedrifts-domener) og for malformert input. *Verifikasjon:* `lib/auth/disposableEmail.test.ts` grønn, `it.each` dekker begge klasser + casing + ingen-`@`.
- [ ] **K2 — Block når self-reg PÅ.** Med `NEXT_PUBLIC_ALLOW_SELF_REGISTRATION=true` og disposable-domene redirecter `sendCode` til `?error=disposable_email` UTEN å kalle `email_is_invited`-RPC eller `signInWithOtp`. *Verifikasjon:* ny test i `actions.test.ts` asserter redirect + `expect(signInWithOtpMock).not.toHaveBeenCalled()`.
- [ ] **K3 — Block uavhengig av invitasjon.** Samme som K2 også når e-posten er invitert (`email_is_invited`→true gjøres aldri pga. short-circuit, men testen bekrefter at invitert+disposable fortsatt blokkeres). *Verifikasjon:* dedikert test.
- [ ] **K4 — Ingen regresjon når self-reg AV.** Med flagget `false` + disposable-domene gjøres INGEN disposable-redirect; flyten fortsetter til RPC/OTP (eksisterende `user_not_found`-sti). *Verifikasjon:* test asserter at `disposable_email`-redirect IKKE skjer.
- [ ] **K5 — Normal e-post uberørt.** Self-reg PÅ + normal e-post → uendret flyt (RPC + OTP kalt). *Verifikasjon:* eksisterende self-reg-tester forblir grønne.
- [ ] **K6 — Norsk melding vist.** `disposable_email`-koden mapper til vennlig norsk banner-tekst (humanizer-kjørt). *Verifikasjon:* `ERROR_MESSAGES`-oppføring finnes; tekst lest.
- [ ] **K7 — IP-tak uendret + dokumentert.** `consumeLoginRateLimit` `ipMax` default = 10 uendret; beslutning dokumentert. *Verifikasjon:* `git diff` viser ingen endring i `loginRateLimit.ts` rate-verdier; closing-kommentar forklarer.
- [ ] **K8 — Gates grønne.** Typecheck + co-located tester + lint passerer; version bumpet + CHANGELOG oppdatert.
