...
- [x] 1. **Test-harness:** `npx jest` i `native/app/` grønn; suiten dekker minst writeScore-mergen, drain-utfallene (applied/server-wins/retry/abandon/mid-flight), mergeServerScore-LWW, playerActions-radtelling og computeState-speilet; `expo-sqlite` mockes av better-sqlite3-adapteren (ekte SQL). Evidens: kjøringslogg + filliste.
  - *Evidens (2026-08-30):* `npx jest` → 11 suiter / 85 tester passed, exit 0. Suiter: writeScore (5), syncWorker (6), realtime/mergeServerScore (6), db-migrasjon (4), gameBundle (5), playerActions (27 inkl. status-guard), seedScores (3), primaryCtaState/roster/formatGate (28), Hole-render (1). Mock: `src/test/sqliteMock.ts` (better-sqlite3 in-memory, `$`-prefiks strippes).
- [x] 2. **Hjem:** simulator mot staging viser e2e-spillerens spill i riktige seksjoner med status-badges; kort åpner GameHome. Evidens: skjermbilde + samsvar med service-role-les av spillerens games.
  - *Evidens (2026-08-30 19:50, simulator 820CA940):* «Pågår nå» viser nøyaktig spillene service-role-lesingen ga (TEST-Cup Singel ×2, TEST-GoldenPath ×2, Byneset North 3. juli) med bane + «Fortsett»-badge; sesjonen overlevde reinstallasjon. Kort → GameHome verifisert. Skjermbilder i økta.
- [x] 3. **Hull-føring lokal-først:** tap på slag i et aktivt staging-spill → øyeblikkelig UI + kø-element; drain lander raden på staging (service-role-les med appens `client_updated_at`); putts-tasting bevarer slag; ekstern `upsert_score_if_newer` vises i appen uten reload (realtime). Evidens: skjermbilder + service-role-les.
  - *Evidens (19:51–19:52, Byneset 9df7b9e0 hull 2):* tre tapp → «3» umiddelbart; ett putte-tapp → 2. Service-role-les: strokes 3, putts 2 (slaget bevart av mergen), `entered_by` = spilleren, `client_updated_at` 17:52:15.265Z (appens stempel). Ekstern RPC satte Bjørns slag 3→6 (17:52:43Z) — appen viste 6 uten reload innen ~9 s. Flighten viste alle 4 spillere (én-flight-regelen), «+1»-badges = delt `strokesForHole` (CH 25/18 · SI 13/7). Rigg-opprydding: Bjørn tilbake til 3.
- [x] 4. **Scorekort + lever:** Layout A-speilet viser 18 hull med par/SI/brutto/netto og totaler konsistente med delt `strokesForHole`; «Lever» setter `game_players.submitted_at` på staging (service-role-les), kø-vakten blokkerer med usynkede slag, manglende hull gir bekreftelses-dialog, og GameHome viser levert-tilstanden.
  - *Evidens (19:53–19:55, GoldenPath d989957f):* CTA-maskinen live: «Start runden» (0 hull) → «Fortsett runden — 1 av 18» etter tasting. Scorekort: 18 rader, hull 1 = Par 5/SI 7/Slag 5/Netto 4 (CH 18 → 1 tildelt slag), totaler 1/5/1/4. «Lever» → dialog «17 hull står uten slag» → «Lever likevel» → staging `submitted_at` 17:54:48Z; GameHome viste «Kortet er levert og godkjent» + «Levert» i rosteret; scorekortet ble lesevisning uten knapp. (Kø-vaktens blokkering er dekket av jest-testene; med nett drainet køen før submit i live-løpet.) Rigg-opprydding: submitted_at nullstilt.
- [x] 5. **Godkjenn:** med et service-role-rigget levert kort fra en flight-makker viser appen kortet under Godkjenn; godkjenn setter `approved_at`+`approved_by_user_id`; avvis nuller `submitted_at` og setter grunn (service-role-les). 0-rader (f.eks. alt godkjent) vises som feil/no-op i UI, aldri stille suksess.
  - *Evidens (19:55–19:57, d989957f, Test Admin rigget levert):* GameHome viste «Godkjenn (1)» etter fokus-refetch. (a) 0-raders-race: kortet ble godkjent EKSTERNT før tapp — appens tapp overskrev ingenting (approved_by forble admin-id-en) og UI-et re-deriverte ærlig til «Ingenting å godkjenne»; {ok:false}-stien er i tillegg jest-låst. (b) Ekte godkjenn: `approved_at` 17:57:14Z, `approved_by_user_id` = spilleren (252e1a6f) — skrevet under 0106-policyen. (c) Avvis m/ grunn: `submitted_at` null, `rejection_reason` «Feil paa hull 2». Rigg-opprydding: alle kolonner nullstilt.
- [x] 6. **Web urørt + porter:** diff kun `native/app/**`, `docs/native/**`, `.forge/**`; `git diff main...HEAD -- lib/` = 0 linjer; alle Gates grønne.
  - *Evidens (2026-08-30 kveld):* `git diff origin/main --name-only` utenfor de tre scopene = tomt; `git diff origin/main -- lib/` = 0 linjer. Gates: app-jest 85/85 exit 0 · app-tsc exit 0 · `expo export` exit 0 (953 moduler, 2.8 MB hbc, dist/ slettet) · rot-typecheck exit 0 · vitest lib/sync+lib/scoring 1303/1303 exit 0 (uendret antall) · `npm run build` exit 0 · `npx eslint native/app` exit 0.
- [x] 7. **Runbook + fysisk iPhone:** `docs/native/app-spike.md` utvidet (navigasjon, skjermene, harness, sqlite-mock, format-gaten); eier-utført tapp-test på fysisk iPhone av hjem→spill→hull→scorekort — inkl. flymodus-føring med cachet bundle. Eier utilgjengelig → dokumentert `VERIFICATION GAP` + restanse, aldri stille hopp.
  - *Evidens (2026-08-30 ~20:36, eierens iPhone 13, post-merge):* runbooken var utvidet i PR #1826. Eieren gjennomførte tapp-testen selv: hjem → spill → hull-tasting → scorekort, inkl. flymodus — «funket på telefonen, flymodus-slagene synket av seg selv» (eier-sitat). Service-role-les bekrefter flymodus-signaturen i spill ae930e68 (TEST-Cup Singel 1): hull 8–10 tastet offline 18:35:08–16Z (`client_updated_at` spredt over ~8 s) og synket i ÉN drain-salve 18:35:41Z (`updated_at`) ved nett-retur — hullene før (4–7) og etter (11–18) har <2 s gap (online). Offline-gapet står i selve dataene, samme mønster som N2-beviset.

## Gates

- [x] `npx jest` i `native/app/` grønt (ny) — 11 suiter / 85 tester, exit 0 (2026-08-30)
- [x] `npx tsc --noEmit` i `native/app/` grønt — exit 0
- [x] `npx expo export --platform ios` grønt (`dist/` slettes etterpå) — exit 0, 953 moduler / 2.8 MB hbc
- [x] `npm run typecheck` (rot) grønt — exit 0
- [x] `npx vitest run lib/sync lib/scoring` grønt (web-suitene uendret) — 55 filer / 1303 tester, exit 0 m/ pipefail
- [x] `npm run build` (rot) grønt før PR — exit 0 (hovedøkta, kveldskjøring)
