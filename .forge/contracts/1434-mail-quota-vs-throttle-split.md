...
1. [ ] Ny Type A-test i `actions.test.ts`: `signInWithOtpMock` returnerer
   `{ error: { message: 'email rate limit exceeded' } }` fra steg 1 →
   `lastRedirect()` er `/login?email=…&error=rate_limited_quota` (INGEN `step=verify`).
2. [ ] Tilsvarende med `from=verify` i formdata → redirect
   `/login?step=verify&email=…&error=rate_limited_quota` (errorCtx-stien, ikke
   overstyring).
3. [ ] Eksisterende throttle-test («For security purposes …» → `rate_limited_minute` +
   `step=verify`) består uendret.
4. [ ] `/login?error=rate_limited_quota&email=x@y.no` rendrer den nye no-copyen (ikke
   «noe gikk galt»-fallbacken) — verifiseres på staging via URL-param på begge steg.
5. [ ] `messages/no.json` + `messages/en.json` har begge nøkkelen
   (`catalogParity`-testen grønn).
6. [ ] `npm run build` grønn (unionen + KNOWN_ERROR_CODES konsistente).

## Gates

- [ ] `npx vitest run "app/[locale]/(auth)/login"` — co-lokaliserte tester grønne
- [ ] `npm run build` (full gate, jf. bindings §T2 — ikke bare tsc)
- [ ] `npm run lint`
- [ ] Staging-klikkrunde: kriterium 4 (URL-drevet render-sjekk; kvoten kan ikke trigges
      reelt på staging uten å brenne den — URL-param-rendringen ER den brukersynlige
      flaten, og rutingen er dekket av Type A-testene)
