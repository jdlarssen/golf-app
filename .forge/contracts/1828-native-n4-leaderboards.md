...
- [x] 7. **Web urørt + porter + runbook + iPhone:** diff-scope som N3 (`lib/` null diff); alle Gates grønne; runbook-seksjon for N4 (leaderboard, gate-endringene, seed-oppskriften); eier-tapptest på fysisk iPhone (greensome-føring + leaderboard, revidert fra scramble). Eier utilgjengelig → `VERIFICATION GAP` + restanse.
  - *Evidens (2026-08-31, eierens iPhone 13, post-merge):* diff-scope verifisert (kun `native/app/**` + docs/forge; `lib/` null diff), alle Gates grønne, runbook-seksjonen skrevet (inkl. seed-oppskrift og OTP-fiks). Eier-tapptesten gjennomført: greensome-føring + Resultater-duellen — «funket, duellen oppdaterte seg live på telefonen» (eier-sitat). Eier-funn under testen: duellkortet klipper lange side-navn — bokført som #1842 (ren layout, påvirker ikke kriteriet). Delt-simulator-læring bokført: 820CA940 ble overtatt av #1830-øktas bygg midt i verifiseringen (samme bundle-id); N4-verifiseringen ble fullført på 498CF5EF med autonom OTP-innlogging.
