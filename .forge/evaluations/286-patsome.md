...
| **K5 — Migrasjon + tee-starter-tabell** | ✅ PASS | `0061_patsome.sql`: tabell m/ PK `(game_id, team_number)` + 4 RLS-policies (read=enhver deltaker, insert/update/delete=lag-medlem ELLER admin) + `updated_at`-trigger + format-seed (`is_active true`, `is_cup_eligible false`) + intent-mapping (`klubb`, `is_primary false`, sort 90). Renummerert fra `0055` til `0061` etter parallell-merge av round_robin (unngår kollisjon). `database.types.ts` håndskrevet (akseptert per kontrakt). |
