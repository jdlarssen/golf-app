...
const STORAGE_PREFIX = 'torny-bbb-podium-confetti-seen-';

export interface BingoBangoBongoPodiumProps {
...
/**
 * Finished-state view for Bingo Bango Bongo — feirings-podium ved
 * `game.status === 'finished'`. Speiler WolfPodium tett med disse
 * forskjellene:
 *   - Hoved-tallet er `totalPoints` (poeng, ikke slag).
 *   - Sub-tittel: «Bingo Bango Bongo · Poeng».
 *   - Topp 3 vises som podium-trinn + resten i flat liste under.
 *   - Distinkt sessionStorage-key: `torny-bbb-podium-confetti-seen-${gameId}`.
 */
export function BingoBangoBongoPodium({
  gameId,
  gameName,
  result,
  playersById,
  backHref = '/',
  chromeless = false,
}: BingoBangoBongoPodiumProps): JSX.Element {
    ...
    <LeaderboardShell chromeless={chromeless}>
      ...
      <div className="px-6 pt-1.5 pb-3.5 text-center">
        ...
        <p className="mt-1 text-[11.5px] tabular-nums text-muted">
          {t('bingoBangoBongo.podiumSubtitle')}
        </p>
      </div>

      <div
        data-testid="bbb-podium"
                ...
                <div className="flex items-center gap-3.5 rounded-2xl border border-border bg-surface px-4 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-[12px] text-muted tabular-nums">
                      {player.bingos} / {player.bangos} / {player.bongos}
                    </p>
                  </div>
