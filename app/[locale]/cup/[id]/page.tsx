...
import { canSelfWithdrawFromCup } from '@/lib/cup/cupSelfWithdrawLink';
...
export default async function PublicCupPage({
  ...
  // spilleren selv (regelen bor i `canSelfWithdrawFromCup`).
  ...
  const myPendingMatches = canSelfWithdrawFromCup({
    userId,
      ...
      {myPendingMatches && (
        <div className="pt-4 pb-2">
          <SmartLink
            ...
            className="flex min-h-[44px] items-center justify-center text-center text-xs text-muted underline underline-offset-2 decoration-muted/40 transition-colors hover:text-text"
          >
