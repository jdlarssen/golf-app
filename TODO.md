...
- [ ] **BUG: Admin-invitasjons-notifikasjons-mail (Resend) sendes ikke i prod.** Symptom: Jørgen inviterte `karlerik.kvilt.holm@gmail.com` 2026-05-13, rad ble lagt til (status "venter" vises), men ingen «Du er invitert»-mail ankom. Status "venter" er forventet UX (det betyr "invitéen har ikke logget inn ennå"), men selve notifikasjons-mailen mangler. OTP-migrasjonen (`6193948`) endret admin-invite fra `supabase.auth.signInWithOtp` (Supabase SMTP) til direkte Resend-SDK-kall (`lib/mail/inviteNotification.ts`). Send-feilen fanges i `app/admin/invitations/actions.ts:58` med kun `console.error` — admin ser fortsatt «Invitasjon sendt»-banner uansett. Hovedhypoteser å verifisere (krever Vercel/Resend-tilgang): (a) `RESEND_API_KEY` eller `RESEND_FROM_EMAIL` ikke satt i Vercel Production env, (b) sender-domenet ikke verifisert i Resend, (c) mail havner i spam. Krever systematisk debug før fix. Når root cause er bekreftet: kompletter med UX-forbedring (vis ekte feil-banner ved send-feil i stedet for "sent"-banner som lyver).
