...
// shot-et og spiller alternate shot derfra. Lag-score per hull → sammenlikn
// side 1 vs side 2 som matchplay (3&2, 2up, AS). Storage følger foursomes-/
// Texas-mønsteret: lag-kapteinen (lex-min userId) eier scores-radene. Reuser
// FoursomesMatchplayResult slik at all leaderboard-/podium-/scorekort-visning
// gjenbrukes uendret.
//
// Eneste forskjell fra foursomes i scoring-laget: lag-handicapet. Greensome
// bruker 60/40-blandingen (60 % av laveste + 40 % av høyeste, avrundet), ikke
// summen — samme formel som greensome-segmentet i patsome (#286). Deretter samme
// diff-baserte allowance:
//   highSideExtraHCP = round(|side1CH − side2CH| × allowance_pct / 100)
// WHS-default allowance = 100 % for greensome (mot 50 % for foursomes).
//
// Ingen tee-starter-feature: begge slår ut hvert hull, så det finnes ingen fast
// odd/even-rotasjon å spore (i motsetning til foursomes).
 ...
 * avrundet (WHS-greensome-konvensjon). Skiller seg fra foursomes (sum).
 */
...
export function compute(ctx: ScoringContext): FoursomesMatchplayResult {
  ...
  // Greensome krever EKSAKT 2 spillere per side. Avvik → defensiv empty shell.
  ...
  // Greensome: lag-HCP = round(0.6×laveste + 0.4×høyeste), ikke sum.
  const side1Combined = greensomeTeamHandicap(
    side1Players[0].courseHandicap,
    side1Players[1].courseHandicap,
  );
  const side2Combined = greensomeTeamHandicap(
    side2Players[0].courseHandicap,
    side2Players[1].courseHandicap,
  );

  // Diff-formel: høylaget får (diff × allowance_pct/100) strokes via SI.
