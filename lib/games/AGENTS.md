...
Alle game-side-konsumenter leser fra cachen (hull-page, scorecard, submit, approve, game-home, leaderboard). Alle mutasjoner (server-actions og route handlers) kaller `expireGameCache(id)` fra `lib/games/expireGameCache.ts` (cupens `tournament-${id}`: `expireTournamentCache`). Hjelperen er `revalidateTag(tag, { expire: 0 })` (Next.js 16 to-arg-form; single-arg er deprecated): `'max'` er stale-while-revalidate og ga gammel status ved første besøk (#2068), og `updateTag` kaster utenfor Server Actions. Vakt-testen ved siden av feiler på direkte `revalidateTag` på disse taggene. `revalidateTag` kaster under render-fase, så server-components (auto-start-fallbacken i game-home) pakker kallet i `after()`.
